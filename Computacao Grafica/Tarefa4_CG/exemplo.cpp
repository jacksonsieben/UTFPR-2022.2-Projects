...
GLfloat escalaEX=1;                  // variável para escala
GLfloat rotacaoEX=0;                 // variável para rotação
GLfloat movexEX = 0, moveyEX = 0;      // variáveis de movimentação para x e y
...
void desenhaObjetoEX(void){                 //função de desenho
    ...
    glPushMatrix();                           //insere a matriz de transformação corrente na pilha
        glScalef(escalaEX, escalaEX, 0);          //chama a fução escala e passa como parâmetro o valor da escala ( as mesmas para x e y)
        glRotatef(rotacaoEX, 0,0,1);            //chama a fução rotação e passa como parâmetro o ângulo
        glTranslatef(movexEX, moveyEX,0);         //chama a fução de tranlação e passa como parâmetro o valor para transladar em x e y
        quadradoEX();                           // chama a função que desenha um quadrado
...
void teclasEX(unsigned char tecla, GLint x, GLint y){
    switch(tecla){
        ...
        break;
        case '1': rotacaoEX-=10;      // rotaciona o objeto no sentido horário (diminui o ângulo em 10)
        break;
        case '2': rotacaoEX+=10;      // rotaciona o objeto no sentido anti-horário (aumenta o ângulo em 10)
        break;
    }
...
int mainEX(void){
    ...
    glutDisplayFunc(desenhaObjetoEX);                                     // função de callback - chama a função Desenha
    glutKeyboardFunc(teclasEX);                                           // Chamada da função teclado (cada pressionamento gera uma chamada do teclado)
    glutSpecialFunc(setasDirecionaisEX);                                  // Chamada da função teclado especial (para quando as teclas direcionais são pressionadas)
